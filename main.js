...
function render(msg=""){let p=location.hash.slice(1)||"dashboard",body=p==="dashboard"?dashboard():p==="upload"?upload():p==="produk"?products():p==="iklan"?ads():p==="roas"?roas():p==="operasional"?ops():detail();document.querySelector("#root").innerHTML=nav(p)+body;bind();if(msg)alert(msg)}
function bind(){
